...
RB_MS1_List<-RB_MS1_List_filtered
write.csv(RB_MS1_List,"RB_MS1_List_noBiotransformer.csv")
...
#Yun-Chung code; find the RB-list metabolites in DEM space
...
# 删除重复的feature，保留唯一alignment所对应MS/MS
...
duplicated(df$name)
duplicated(df$AID)
# 定义处理函数，用于将字符串拆分成两列，并使用指定的名称命名数据框
...
# 第一步：根据条件1 (Metabolite.name == Source 且 Metabolite_origin == Target) 进行合并
...
# 第二步：根据条件2 (Metabolite.name == Target 且 Metabolite_origin == Source) 进行合并
...
# 第五步：删除所有重复metabolite_annotation;留下distance最高者
MS2FeatureList_unique <- MS2FeatureList %>%
  group_by(Alignment.ID) %>%     # 按Alignment.ID分组
  slice_max(order_by = Distance) %>%  # 保留Distance的最大值对应的行
  ungroup()  # 取消分组
MS2FeatureList_unique <- MS2FeatureList_unique %>%
  distinct(Alignment.ID, .keep_all = TRUE) %>%  # 根据 Alignment.ID 去重
  arrange(Alignment.ID)                        # 按 Alignment.ID 排序以保持原有顺序
...
MS2FeatureList_unique_plusMZRT$after_colon <- sub(".*:(.*)", "\\1", MS2FeatureList_unique_plusMZRT$Metabolite.name)
split_elements <- unlist(strsplit(MS2FeatureList_unique_plusMZRT$after_colon, "(?<=\\w)(?=[+-])", perl=TRUE))
filtered_elements <- split_elements[!grepl("-[123]", split_elements)]
element_counts <- table(filtered_elements)
print(element_counts)
write.csv(element_counts,"PG1reactionType.csv")
...
MS2FeatureList_unique_Merged <- MS2FeatureList_unique_Merged %>%
  ...
  filter(!(n() > 1 & grepl(":", Metabolite.name))) %>% # 删除重复的包含 ':' 的行
  slice(1) %>% # 如果不包含 ':', 只保留第一行
  ungroup()
...
highconfidenceMetabolite <- highconfidenceMetabolite %>%
  mutate(
    temp = ifelse(grepl(":", Source), Source, Target), # 暂存 Source 或 Target 的值
    Source = ifelse(grepl(":", Source), Target, Source), # 更新 Source
    Target = temp # 将暂存值赋给 Target
  ) %>%
  select(-temp) # 删除临时列

HC_Metabolite_list<- MS2FeatureList %>%
  inner_join(highconfidenceMetabolite, by = c("Metabolite.name" = "Target"))
# 假设 HC_Metabolite_list 已有 Metabolite.name 列
HC_Metabolite_list <- HC_Metabolite_list %>%
  mutate(Ion_Form = "[M+H]+") # 默认赋值
# 找出 DistMatrix_edge_Final_ISF 中 Source 和 Target 都在 HC_Metabolite_list$Metabolite.name 里的行
...
# 更新 HC_Metabolite_list 的 Ion_Form 列
HC_Metabolite_list <- HC_Metabolite_list %>%
...
#Correlation check
#single-reaction
SpectrumSimilarity(list_of_MSMSSpectrum[[173]],list_of_MSMSSpectrum[[182]],t = 0.25, b = 10, 
                   top.label = "Bensulide:-S+O", bottom.label = "Bensulide", 
                   xlim = c(50, 350), x.threshold = 0, 
                   print.alignment = FALSE, print.graphic = TRUE,
                   output.list = FALSE)

msentropy_similarity(list_of_MSMSSpectrum[[173]], list_of_MSMSSpectrum[[182]], 
                     ms2_tolerance_in_da = 0.02,
                     clean_spectra = TRUE,
                     min_mz = 0,
                     max_mz = 1000,
...
SpectrumSimilarity(list_of_MSMSSpectrum[[107]],list_of_MSMSSpectrum[[28]],t = 0.25, b = 10, 
                   top.label = "Isoxadifen-ethyl:-C2H4-H2O", bottom.label = "Isoxadifen-ethyl", 
                   xlim = c(50, 350), x.threshold = 0, 
                   print.alignment = FALSE, print.graphic = TRUE,
                   output.list = FALSE)
msentropy_similarity(list_of_MSMSSpectrum[[107]], list_of_MSMSSpectrum[[28]], 
                     ms2_tolerance_in_da = 0.02,
                     clean_spectra = TRUE,
                     min_mz = 0,
                     max_mz = 1000,
                     ...
                     max_peak_num = 100,
                     weighted = TRUE,)
SpectrumSimilarity(list_of_MSMSSpectrum[[146]],list_of_MSMSSpectrum[[32]],t = 0.25, b = 10, 
                   top.label = "Isoxadifen-ethyl:-C2H4+NH3", bottom.label = "Isoxadifen-ethyl", 
                   xlim = c(50, 350), x.threshold = 0, 
                   print.alignment = FALSE, print.graphic = TRUE,
                   output.list = FALSE)
msentropy_similarity(list_of_MSMSSpectrum[[146]], list_of_MSMSSpectrum[[32]], 
                     ms2_tolerance_in_da = 0.02,
                     clean_spectra = TRUE,
                     min_mz = 0,
                     max_mz = 1000,
